...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Call Register API — backend sends OTP on success (202)
        setIsLoading(true);
        setError("");
        setSuccess("");
        ...
        try {
            ...
            if (response.success) {
                // 202 Accepted — OTP sent to email, open verification modal
                setSuccess(response.message || "Check your email for OTP.");
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            const response = await authApi.verifyEmail(formData.email, otpValue);
            ...
            if (response.success) {
                ...
                // Show exact backend error message — fully dynamic
    ...
    const handleResendOtp = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                // Show exact backend error (e.g., RATE_LIMIT_EXCEEDED message)
                setError(response.error || "");
            }
        } catch (err: any) {
        ...
        <div className="h-screen w-screen overflow-hidden flex flex-col lg:flex-row items-stretch bg-white dark:bg-slate-900">
            ...
            <AnimatePresence>
                {showOtpModal && (
                    ...
                    >
                        <motion.div
                            className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl p-6 relative overflow-hidden text-center"
                        ...
                        >
                            ...
                            <div className="flex justify-center gap-2 mb-6">
                                {otp.map((digit, index) => (
                                    <input
                                        ...
                                        className="w-10 h-12 rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-center text-lg font-bold text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all"
                                    />
                                ))}
                            </div>
                            ...
                            {(error || success) && (
                                <div className="mb-4 text-xs font-medium">
                                    {error ? (
                                        <span className="text-red-500 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-full">{error}</span>
                                    ) : (
                                        <span className="text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1.5 rounded-full">{success}</span>
                                    )}
                                </div>
                            )}
                            ...
                            <button
                                ...
                                className="w-full h-11 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-sm transition-all shadow-md shadow-emerald-500/20 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                                {isVerifying ? (
                                    <>
                                        <Loader2 className="w-4 h-4 animate-spin" />
                            ...
                            <div className="mt-4 text-xs text-slate-500">
                                ...
                                <button
                                    ...
                                    className="font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 hover:underline transition-colors"
                                >
                                    Resend Code
                                </button>
                            </div>
